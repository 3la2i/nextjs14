  ...
  const fetchLikes = async (postId) => {
    ...
    MySwal.fire({
      ...
      preConfirm: async (reason) => {
        if (!reason) {
          Swal.showValidationMessage("Please select a reason to report");
          return false;
        }
        try {
          const response = await axios.patch(
            `/api/postsCommunity/${selectedPost._id}/report`, // Ensure this is the correct path
            { reason, userId },
            { withCredentials: true }
          );
  
          if (response.status !== 200) {
            throw new Error(response.data.message || "An error occurred while reporting");
          }
      ...
      if (result.isConfirmed) {
        MySwal.fire({
          ...
          text: "We will review this post based on your feedback.",
        });
      }
    });
  };
    ...
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="relative h-64 mb-12 rounded-lg overflow-hidden">
        ...
        <div className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center">
          <h1 className="text-5xl font-bold text-[#bee3b2f3]">Environmental Community</h1>
        </div>
      </div>
      <div className="container mx-auto px-4 py-8">
        <button 
          onClick={() => setIsPopupOpen(true)} 
          className="bg-[#a2e08ff3] text-black px-6 py-2 rounded-full flex items-center mb-12 hover:bg-green-700 transition duration-300"
        >
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
            <div className="bg-white rounded-lg p-6 w-96 relative">
              ...
              <form onSubmit={handleSubmit}>
                ...
                <button 
                  type="submit" 
                  className="bg-[#588b49f3] text-white px-6 py-2 rounded-full flex items-center hover:bg-[#4c773ff3] transition duration-300"
                >
          ...
          {filteredPosts.map(post => (
            ...
            >
              ...
              <div className="p-4">
                ...
                <p className="text-sm text-gray-600 mb-4">{post.tags}</p>
